...
void ChangeSceneLoad_10(DWORD_PTR, DWORD_PTR);
void ExitGame_10(DWORD_PTR, DWORD_PTR);
void ResumeGame_10(DWORD_PTR, DWORD_PTR);

bool bSettingOn_10 = false;

CScene_Stage_10::CScene_Stage_10()
{
...
void CScene_Stage_10::update()
{
	CScene::update();
// 	if (KEY_TAP(KEY::ENTER))
// 	{
// 		ChangeScene(SCENE_TYPE::STAGE_11);
// 	}

	if (KEY_TAP(KEY::ESC) && !bSettingOn_10)
	{
		//SettingUI
		m_pUI = new CPanelUI;
		...
		pResumeBtn->SetClickedCallBack(ResumeGame_10, 0, 0);
		...
		pLoadGameBtn->SetClickedCallBack(ChangeSceneLoad_10, 0, 0);
		...
		pExitGameBtn->SetClickedCallBack(ExitGame_10, 0, 0);
...
void CScene_Stage_10::Enter()
{
	...
	// TileObject 배치
	CreateTile(64, 64);
	wstring szname = L"tile\\Stage10.tile";
	LoadTile(szname);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		pLazer->SetPos(Vec2(608.f, 1440.f + 64.f * i));
	...
	for (int i = 0; i < 8; i++)
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetName(L"Lazer");
		pLazer->SetPos(Vec2(1440.f, 1888.f + 64.f * i));
		pLazer->SetScale(Vec2(64.f, 64.f));
	...
	for (int i = 0; i < 9; i++)
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetName(L"Lazer");
		pLazer->SetPos(Vec2(736.f + 64.f * i, 2464.f));
		pLazer->SetScale(Vec2(64.f, 64.f));
		pLazer->SetOwnerLever((CLever*)pLever1);
		AddObject(pLazer, GROUP_TYPE::LAZER);
	}
