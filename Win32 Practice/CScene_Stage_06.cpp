...
void ChangeSceneLoad_06(DWORD_PTR, DWORD_PTR);
void ExitGame_06(DWORD_PTR, DWORD_PTR);
void ResumeGame_06(DWORD_PTR, DWORD_PTR);

bool bSettingOn_06 = false;

CScene_Stage_06::CScene_Stage_06()
{
...
// 	}
	...
	if (KEY_TAP(KEY::ESC) && !bSettingOn_06)
	{
		//SettingUI
		m_pUI = new CPanelUI;
		...
		pResumeBtn->SetClickedCallBack(ResumeGame_06, 0, 0);
		...
		pLoadGameBtn->SetClickedCallBack(ChangeSceneLoad_06, 0, 0);
		...
		pExitGameBtn->SetClickedCallBack(ExitGame_06, 0, 0);
...
void CScene_Stage_06::Enter()
{
	...
	// TileObject 배치
	CreateTile(64, 64);
	wstring szname = L"tile\\Stage06.tile";
	LoadTile(szname);
	...
	pLever->SetPos(Vec2(17600.f, 16320.f));
	...
	for (int i = 0; i < 4; i++)
	{
		...
		pLazer->SetPos(Vec2(2528.f, 864.f + 64.f * i));
	...
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetPos(Vec2(2528.f, 1248.f + 64.f * i));
		pLazer->SetScale(Vec2(64.f, 64.f));
		(pLazer)->SetOwnerLever((CLever*)pLever);
		pLazer->SetIdx(1);
		AddObject(pLazer, GROUP_TYPE::LAZER);
	...
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetPos(Vec2(3040.f, 1440.f + 64.f * i));
		pLazer->SetScale(Vec2(64.f, 64.f));
		(pLazer)->SetOwnerLever((CLever*)pLever);
		pLazer->SetIdx(1);
		AddObject(pLazer, GROUP_TYPE::LAZER);
	...
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetPos(Vec2(3040.f, 1056.f + 64.f * i));
		pLazer->SetScale(Vec2(64.f, 64.f));
		(pLazer)->SetOwnerLever((CLever*)pLever);
		pLazer->SetIdx(1);
		AddObject(pLazer, GROUP_TYPE::LAZER);
