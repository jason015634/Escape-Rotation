...
void ChangeSceneLoad_07(DWORD_PTR, DWORD_PTR);
void ExitGame_07(DWORD_PTR, DWORD_PTR);
void ResumeGame_07(DWORD_PTR, DWORD_PTR);

bool bSettingOn_07 = false;

CScene_Stage_07::CScene_Stage_07()
{
...
// 	}
	...
	if (KEY_TAP(KEY::ESC) && !bSettingOn_07)
	{
		//SettingUI
		m_pUI = new CPanelUI;
		...
		pResumeBtn->SetClickedCallBack(ResumeGame_07, 0, 0);
		...
		pLoadGameBtn->SetClickedCallBack(ChangeSceneLoad_07, 0, 0);
		...
		pExitGameBtn->SetClickedCallBack(ExitGame_07, 0, 0);
...
void CScene_Stage_07::Enter()
{
	...
	// TileObject 배치
	CreateTile(64, 64);
	wstring szname = L"tile\\Stage07_2.tile";
	LoadTile(szname);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		pLazer->SetPos(Vec2(1632.f + 64.f * i, 1824.f));
	...
	for (int i = 0; i < 9; i++)
	{
		CLazer* pLazer1 = new CLazer;
		pLazer1->SetPos(Vec2(2592.f, 1504.f + 64.f * i));
		pLazer1->SetScale(Vec2(64.f, 64.f));
		pLazer1->SetIdx(1);
		(pLazer1)->SetOwnerLever((CLever*)pLever1);
		AddObject(pLazer1, GROUP_TYPE::LAZER);
	}
	// 충돌 지정
