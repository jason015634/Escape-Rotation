...
void ChangeSceneLoad_09(DWORD_PTR, DWORD_PTR);
void ExitGame_09(DWORD_PTR, DWORD_PTR);
void ResumeGame_09(DWORD_PTR, DWORD_PTR);

bool bSettingOn_09 = false;

CScene_Stage_09::CScene_Stage_09()
{
...
void CScene_Stage_09::update()
{
	CScene::update();	
// 	if (KEY_TAP(KEY::ENTER))
// 	{
// 		ChangeScene(SCENE_TYPE::STAGE_10);
// 	}

	if (KEY_TAP(KEY::ESC) && !bSettingOn_09)
	{
		//SettingUI
		m_pUI = new CPanelUI;
		...
		pResumeBtn->SetClickedCallBack(ResumeGame_09, 0, 0);
		...
		pLoadGameBtn->SetClickedCallBack(ChangeSceneLoad_09, 0, 0);
		...
		pExitGameBtn->SetClickedCallBack(ExitGame_09, 0, 0);
...
void CScene_Stage_09::Enter()
{
	...
	pLever->SetPos(Vec2(864.f, 1056.f));
	pLever->SetScale(Vec2(64.f, 64.f));
	AddObject(pLever, GROUP_TYPE::LEVER);

	CObject* pLever1 = new CLever;
	pLever1->SetName(L"Lever");
	pLever1->SetPos(Vec2(1696.f, 1376.f));
	pLever1->SetScale(Vec2(64.f, 64.f));
	((CLever*)pLever1)->SetIdx(2);
	AddObject(pLever1, GROUP_TYPE::LEVER);

	CObject* pLever2 = new CLever;
	pLever2->SetName(L"Lever");
	pLever2->SetPos(Vec2(2720.f, 1888.f));
	pLever2->SetScale(Vec2(64.f, 64.f));
	((CLever*)pLever2)->SetIdx(2);
	AddObject(pLever2, GROUP_TYPE::LEVER);

	for (int i = 0; i < 5; i++)
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetName(L"Lazer");
		pLazer->SetPos(Vec2(2272.f + 64.f * i, 1184.f));
	...
	for (int i = 0; i < 4; i++)
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetName(L"Lazer");
		pLazer->SetPos(Vec2(2592.f, 1248.f + 64.f * i));
		pLazer->SetScale(Vec2(64.f, 64.f));
		pLazer->SetOwnerLever((CLever*)pLever2);
		pLazer->SetIdx(1);
		AddObject(pLazer, GROUP_TYPE::LAZER);
	}
	...
	for (int i = 0; i < 4; i++)
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetName(L"Lazer");
		pLazer->SetPos(Vec2(1952.f + 64.f * i, 1824.f));
		pLazer->SetScale(Vec2(64.f, 64.f));
		pLazer->SetOwnerLever((CLever*)pLever1);
		AddObject(pLazer, GROUP_TYPE::LAZER);
	}
	...
	for (int i = 0; i < 6; i++)
	{
		CLazer* pLazer = new CLazer;
		pLazer->SetName(L"Lazer");
		pLazer->SetPos(Vec2(2272.f + 64.f * i, 1824.f));
		pLazer->SetScale(Vec2(64.f, 64.f));
		pLazer->SetOwnerLever((CLever*)pLever2);
		AddObject(pLazer, GROUP_TYPE::LAZER);
	}
